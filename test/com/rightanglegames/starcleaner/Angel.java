...
/**
 * this class only works with a fixed frame rate of 60. All calculations are based on that assumption.
 * It's okay for this little sample game
 * @author Thomas
 *
 */
...
//				sprPlayer.play("walkRight");
			...
			if (speed.x == 0) {
				if (faceRight)
					...
					currentImage = sheet.getSprite(0, 0).getFlippedCopy(true, false);
//				if (direction) { sprPlayer.play("standRight"); } else { sprPlayer.play("standLeft"); }
			...
			if (faceRight)
				...
				currentImage = sheet.getSprite(2, 0).getFlippedCopy(true, false);
...
//			if (collide(solid, x + 1, y)) { sprPlayer.play("slideLeft"); }
		...
		if (previousx != x || previousy != y)
			myLight.setLocation(x+StarCleaner.TILESIZE/2, y+StarCleaner.TILESIZE/2);
		...
		if (sunOrMoon != null && this.world.getNrOfEntities(Star.STAR_TYPE) == 0) {
