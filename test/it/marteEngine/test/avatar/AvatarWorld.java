...
public class AvatarWorld extends World {
	...
	public void init(GameContainer container, StateBasedGame game)
		...
		bindToKey("followEntity", Input.KEY_F);
		...
		if (pressed("change_mode")) {
			...
			switch (cameraMode) {
				...
				case FOCUS_ON_RIGHT_CLICK :
					...
					if (pressed("focus")) {
						camera.center(input.getMouseX() + camera.getX(),
	...
	private void nextMode() {
		...
		switch (cameraMode) {
			case FOLLOW_ENTITY :
				camera.follow(player, Camera.FollowStyle.TOPDOWN);
	...
	public void nextFollowStyle() {
		Camera.FollowStyle[] styles = Camera.FollowStyle.values();
		Camera.FollowStyle currentStyle = camera.getCurrentFollowStyle();
		Camera.FollowStyle nextStyle = styles[(currentStyle.ordinal() + 1)
				% styles.length];
